...
class Tool:
  ...
  def __init__(self, name, what, object, hint = None, solution = None):
    ...
    what - что проверяем ['df', 'series', 'file', 'single_value', 'array']
    ...
    # =====================================================================TODO==============================================================
    #TODO заменить все замены на одну переменную
    #заменить print на функцию color_text
    # =====================================================================TODO==============================================================
  ...
  def check(self, check_obj = None):
    ...
    if self.what == 'series':
      ...
      if correct:
        ...
        if self._solution:
          print(color_text(f'''`b`Решение автора:`n` 
          {self._solution}'''))
        print()
    ...
    if self.what == 'array':
      ...
      # 2 проверяем сами значения
      if correct:
        if list(self.var) != list(check_obj):
          print('\033[1;31m{} \033[0m{}\033[48;5;252m{}\033[0m {}'.format('Промах :', 'проверьте, пожалуйста, значения в переменной ', self.name, ', они отличаются от требуемых'))
          print()
          correct = False
  ...
  def hint(self):
    ...
    if self._hint:
      ...
      if self.counter_check < 3:
        print(color_text(f'`b`Подсказка:`n` подсказка будет доступна после 3 попыток проверки ответа (пробуйте решить самостоятельно), вы проверили `gf`{self.counter_check}`n` раз'))
      else:
  ...
  def solution(self):
    ...
    if self._solution:
      ...
      if self.counter_check < 5:
        print(color_text(f'`b`Решение:`n` решение будет доступно после 5 попыток проверки ответа (пробуйте решить самостоятельно), вы проверили `gf`{self.counter_check}`n` раз'))
      else:
