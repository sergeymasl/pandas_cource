  ...
  def hint(self):
    ...
    if self._hint:
      ...
      if self.counter_check < 3:
        print(color_text('`b`Подсказка:`n` подсказка будет доступна после 3 попыток проверки ответа (пробуйте решить самостоятельно), вы проверили `gf`{self.counter_check}`n` раз'))
      else:
  ...
  def solution(self):
    ...
    if self._solution:
      ...
      if self.counter_check < 5:
        print(color_text('`b`Решение:`n` решение будет доступно после 5 попыток проверки ответа (пробуйте решить самостоятельно), вы проверили `gf`{self.counter_check}`n` раз'))
      else:
